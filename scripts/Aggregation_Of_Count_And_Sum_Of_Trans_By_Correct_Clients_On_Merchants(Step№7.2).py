...
try:
    ...
    sql_query = ("""
    IF(OBJECT_ID('Aggregation_Of_Count_And_Sum_Of_Trans_By_Correct_Clients_On_Merchants') IS NULL)
        SELECT 0;
    ELSE 
        SELECT 1;
    """)
    ...
    if (existence != 1):
        ...
        sql_query = ("""
        SELECT merchant_id, cli_id, count_trans_client, sum_trans_client
        INTO Aggregation_Of_Count_And_Sum_Of_Trans_By_Correct_Clients_On_Merchants
        FROM Aggregation_Of_Count_And_Sum_Of_Trans_By_Clients_On_Merchants
        WHERE count_trans_client>=4*{0}
        """.format(months))
